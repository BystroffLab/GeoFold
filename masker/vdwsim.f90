...
integer :: ios=0, nat
real :: tt, sig=0.8,x,y,z,nrg,fa,timestep,dd,ddmax,boltzk,kt,nava,ms,d,r,v,sumd
...
jarg = iargc()
if (jarg < 2) then
  write(*,*) 'Usage: xmcmask coordfile boxsize [T N freq outputfile sig lastpdb]'
  write(*,*) 'T=',tt,' kT=',kt
  write(*,*) 'N=',ncycle
...
if (jarg > 6) then
  call getarg(7,aline)
  read(aline,*,iostat=ios) sig
  if (ios/=0) stop 'Bad sig'
  if (sig < 0) stop 'sig must be > 0'
endif
if (jarg > 7) then
...
if (ios/=0) stop 'Error allocating xyz'
allocate(xyzsave(3,nat),stat=ios)
if (ios/=0) stop 'Error allocating xyzsave'
  ...
  if (mod(icycle,nfreq)==0) then
    ...
    do i=1,nat
      ...
      write(13,'("ATOM  ",i5,2x,a1,3x,a3,1x,a1,i4,4x,3f8.3,i9,f7.1)') &
                i,"C", "CH4"," ",i,vec(1:3),icycle,tt
    enddo
    if (.not.drawing) close(13)
  endif
  !! Get surface area energy 
  !! call getms(xyznew,atype,nat,ms)  !! returns ms in kJ/mol
  !! Get VDW
  v = 0.
  do i=1,nat-1
    do j=i+1,nat
...
do i=1,nat
  write(14,'("ATOM  ",i5,2x,a3,1x,a3,1x,a1,i4,4x,3f8.3,i9,f7.1)') &
            i,"C", &
            "CH4"," ",i,xyz(1:3,i),0,tt
enddo
close(14)
stop 'mcmask v.05-MAR-02'
CONTAINS
!----------------------------------------------------!
