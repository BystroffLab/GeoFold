...
!*=========================================================================
implicit none
integer,parameter :: MAXATOM=1024,MAXPSI=360
...
do iat=1,nat
  ...
  sasa = sasa + x
  call drawsurface(13,xyz(1:3,iat),r1,iat,amask,50.,"C",20)  !! output the sasa surface
  i = 0
  cmask = 0
  nn = 0
  do k=1,nm
    ...
    do m=1,nn
      ...
      if (any(bmask /= 0)) then !! if there is at least one vertex
        ...
        if (any(emask /= 0)) then
          ...
          call drawsurface(13,wxyz(1:3),rw,iat,emask,-20.,"E",20) !! output the surface
        endif
