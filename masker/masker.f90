  !! Module version of pdbmask.f90 (Thu Jun 14 12:00:29 EDT 2007 C.Bystroff)
  ...
  !** ====================================================================
  !** Copyright 2001-2008 Chris Bystroff
  !**
  ...
  real,public :: DTHETA   !! degree increments
  ...
  real,public :: rw=1.4   !! not private
  real :: wsphere, sasa, ssasa, bsasa
  integer,dimension(:),allocatable,target,public :: atype
  ...
  logical,private :: drawing=.false., use_gromacs=.false.
  !!!WZS things
  logical,public :: wzs = .true.
  character(len=6),dimension(30) :: wzsTypes
  real, dimension(30) :: wzsr,wzsw,wzsm
  !!!
  logical :: periodic,rendering=.false.,smoothing=.false.,masker_saying=.false.
  logical,private :: savecollars=.false.
  ...
  type(atomtype),dimension(:),allocatable,public :: atomlib
  integer,public :: nattype
  public masker_getms,masker_initmasks,masker_allocatoms,masker_getatypeptr,masker_deallo
  public masker_initcollars, masker_getvmask, masker_getmxyz,masker_plot,masker_render,masker_plotting
  public masker_getimask,masker_maskj,masker_drawsurface,masker_setlbox,masker_renderheader,masker_usage
  public masker_nmask,masker_dpsi,masker_setcoloring,masker_saying,masker_getwaters
  !!INTERFACE
...
CONTAINS
!=========================================================================!
  subroutine masker_usage    !! show current environment variables
    ...
    call getenv("FORCEFIELD",aline); write(*,*) "FORCEFIELD=",trim(aline)
    ...
    call getenv("DTHETA",aline); write(*,*) "DTHETA=",trim(aline)
  end subroutine masker_usage
!-------------------------------------------------------------------------!
...
!-------------------------------------------------------------------------!
  ...
  character(len=200) :: aline,pdbfile,binfile,logfile,outfile,template,cbitfile
  character(len=200) :: libfile
  character(len=200) :: vmaskfile,forcefile,vtriangle,collarfile,slopefile,fmt
  character(len=1000) :: design_dir

  integer :: i,j,k,L,m,jarg,ios,iargc,natm,kcc=KC,ii
  ...
  !! ln -s /home/database/masker/4096.mask  4096.mask
  !!
  !! Default settings:
  !!
  !!  setenv  MASKLIB  4096.mask
  ...
  if (design_dir=="") call maskersetuperror("DESIGN_HOME not set")
  write(aline,*) MAXATOM
  aline=adjustL(aline)
  call getenv("MASKLIB",binfile)
  if (binfile=="") binfile = trim(design_dir) // "/" // trim(aline) // ".mask"
  call getenv("VMASK",vmaskfile)
  if (vmaskfile=="") vmaskfile = trim(design_dir) // "/" // trim(aline) // ".vmask"
  call getenv("VTRIANGLE",vtriangle)   !! triangle indeces
  if (vtriangle=="") vtriangle = trim(design_dir) // "/" // trim(aline) // ".tri"
  call getenv("MASKTEMPLATE",template)
  if (template=="") template = trim(design_dir) // "/" // trim(aline) // ".pdb"
  call getenv("MASKDAT",logfile)
  if (logfile=="") logfile = trim(design_dir) // "/" // trim(aline) // ".dat"
  call getenv("COLLARS",collarfile)
  if (collarfile=="") collarfile = trim(design_dir) // "/" // "collars" // trim(aline) // ".bin"
  call getenv("COUNTBITS",cbitfile)
  if (cbitfile=="") cbitfile = trim(design_dir) // "/" //"cbits.bin"
  call getenv("FORCEFIELD",forcefile)
  if (forcefile=="") forcefile = trim(design_dir) // "/" // "forcefield.prm"
  call getenv("ATOMLIB",libfile)
  if (libfile=="") libfile = trim(design_dir) // "/" //"atoms.lib"
  call getenv("DTHETA",aline)
  if (aline/="") then
    read(aline,*,iostat=ios) DTHETA
    if (ios/=0) call maskersetuperror("parsing DTHETA")
  endif
  write(aline,'(f3.1)') DTHETA
  aline=adjustL(aline)
  call getenv("SLOPES",slopefile)
  if (slopefile=="") slopefile = trim(design_dir) // "/" // "slopes" // trim(aline) // ".dat"
  !! dependent constants
  ...
  if (ios/=0) then
     write(0,*) trim(cbitfile)
     call maskersetuperror('COUNTBITS file missing')
  endif
  ...
  !! ====================================================================
  !! FORCEFIELD
  !! Read atom types, radii, weights, etc. from FORCEFIELD file
  !! Data from GROMACS
  !! This option must be debugged/validated before using!!! 
  !!---------------------------------------------------------------
  if (use_gromacs) then
  ...
  !!! WZS stuff
    ...
    nattype = 30
    wzsTypes = (/"COODEC","CNHC  ","CNKC  ","CNRC  ","COSTC ","CONQNC",&
                 "CP    ","Calpha","COYC  ","CSMC  ","CONC  ","CNWC  ",&
                 "Caliph","CW    ","CF    ","CSCC  ","CNHN  ","CNKN  ",&
                 "CNRN  ","CONQNN","CONN  ","CNWN  ","COODEO","COSTO ",&
                 "CONQNO","COYO  ","CONO  ","CSMS  ","CSCS  ","CY    "/)
    wzsm(1:16)=16.000
    wzsm(17:22)=14.000
    wzsm(23:27)=16.000
    wzsm(28:29)=32.000
    ...
    wzsr(28:29)=2.200
    wzsr(30)=1.840
    wzsw = (/-45.9445,-13.8909,-51.1661,-30.3717,-3.9162,5.8534,20.8949,11.2466,&
             13.8700,38.8275,14.5017,25.7107,41.6768,50.4172,74.8099,120.2482,&
             -13.8909,-51.1661,-30.3717,5.8534,14.5017,25.7107,-45.9445,-3.9162,&
             5.8534,13.8700,14.5017,38.8275,120.2482,26.9408/)
    allocate(atomlib(30))
    do i = 1, 30
      atomlib(i)%name = wzsTypes(i)
      atomlib(i)%m = wzsm(i)
      atomlib(i)%r = wzsr(i)
      atomlib(i)%w1 = wzsw(i)
      atomlib(i)%w2 = wzsw(i)
  ...
  !!---------------------------------------------------------------
    ...
    if (ios/=0) stop 'masker_initmasks - ERROR: missing ATOMLIB file'
    read(junit,*,iostat=ios) nattype
    if (ios/=0) stop 'masker_initmasks - ERROR: reading ATOMLIB file(1)'
    if (allocated(atomlib)) stop 'masker_initmasks - ERROR: please do not call masker_initmasks twice'
    allocate(atomlib(nattype),stat=ios)
    if (ios/=0) stop 'masker_initmasks - ERROR: allocating atomlib()'
    do i=1,nattype
      read(junit,'(a4,5f8.3)',iostat=ios) atomlib(i)%name, atomlib(i)%m, atomlib(i)%r,  &
             atomlib(i)%w1,  atomlib(i)%w2,  atomlib(i)%w3
      if (ios/=0) stop 'masker_initmasks - ERROR: reading ATOMLIB file(2)'
      !! Diagnostic
      !!write(*,'(a4,5f8.3)',iostat=ios) atomlib(i)%name, atomlib(i)%m, atomlib(i)%r,  &
      !!       atomlib(i)%w1,  atomlib(i)%w2,  atomlib(i)%w3
    enddo
    close(junit)
  endif
  ...
  !!
  !!  write(12,'("integer,parameter :: MAXATOM=512    !! number of points in mask.")')
  ...
  open(12,file=vmaskfile,status='old',form='unformatted',iostat=ios)
  if (ios/=0) then
    write(*,*) 'masker_initmasks - ERROR: missing VMASK file'
    vmask = -1
  else
    read(12,iostat=ios) vmask(1:MASKSIZE)
    if (ios/=0) then
      write(*,*) 'masker_initmasks - ERROR: reading VMASK file'
      vmask = -1
    endif
  endif
  close(12)
  ...
  !! --------------- BUG in triangles.f90 ------------------
  !! Until bug is fixed, VTRIANGLE will be shut off.
  !!--------------------------------------------------------
  if (1==1) then
    open(12,file=vtriangle,status='old',form='formatted',iostat=ios)
    if (ios/=0) then
      if (ios==203) then
        write(*,*) 'NOT initializing VTRIANGLE because it is already opened.'
        write(*,*) 'VTRIANGLE=',trim(vtriangle)
      else
        write(*,*) 'masker_initmasks - ERROR: missing VTRIANGLE file, ios=',ios
        write(*,*) 'VTRIANGLE=',trim(vtriangle)
      endif
    else
      ntri = 0
  ...
  ! savecollars = .false.
   do ipsi=0,npsi
     ...
     do iphi=1,nphi(ipsi)
       ...
       if (savecollars) then
         ...
         uvec = -uvec
         imask2 = masker_getimask(uvec)  !! inverse mask index
         do itheta=0,NTHETA-1
           theta = itheta*dpsi
           if (itheta == 0) then
             amask = -1
             !forall (j=1:MASKSIZE/4)
               amask = iand(amask,not(masklib(:,itheta+1,imask)))
               !amask(j) = iand(amask(j),not(masklib(j,itheta+1,imask)))
               !amask(1+4*(j-1):4*j) = iiand(amask(1+4*(j-1):4*j),not(masklib(1+4*(j-1):4*j,itheta+1,imask)))
             !endforall
           else 
             !forall (j=1:MASKSIZE/4)
               amask = iand(masklib(:,itheta,imask),not(masklib(:,itheta+1,imask)))  
               ! amask(j) = iand(masklib(j,itheta,imask),not(masklib(j,itheta+1,imask)))  
               !amask(1+4*(j-1):4*j) = iiand(masklib(1+4*(j-1):4*j,itheta,imask),not(masklib(1+4*(j-1):4*j,itheta+1,imask)))  
             !endforall
           endif
           !write(*,*) amask
           j = countbits(amask)
           ...
           !! Diagnostic
           ! write(*,*) imask,psi,phi,itheta,theta,j
         enddo
         !! Thu Feb  7 14:35:30 EST 2002 
         !! For the interpolation step when atom j is embedded, the collarsize at 90 deg is used, 
         !! get collar for theta = 90 deg by masking 90 deg mask to next-to-last mask
         !! pointing the opposite way.
         amask = iand(masklib(:,NTHETA,imask),masklib(:,NTHETA-1,imask2))  
         !forall (j=1:MASKSIZE/4)
         ! amask = iiand(masklib(:,NTHETA,imask),masklib(:,NTHETA-1,imask2))  
         ! amask(1+4*(j-1):4*j) = iiand(masklib(1+4*(j-1):4*j,NTHETA,imask),masklib(1+4*(j-1):4*j,NTHETA-1,imask2))  
         !endforall
         j = countbits(amask)
   ...
   if (savecollars) then
     ...
     if (ios/=0) stop 'masker_initmasks - ERROR: opening COLLARS file for writing'
  ...
  if (ios/=0) then
    write(*,*) 'masker_initmasks - ERROR: missing COLLARS file'
  else
    read(33,iostat=ios) collarsize(0:NTHETA,1:nmask)
    if (ios/=0) write(*,*) 'masker_initmasks - ERROR: reading COLLARS file'
    close(33)
  endif
  ...
  !! slope() is in units of square angstroms on a unit sphere, per radian.
  !! Mon Dec 17 12:47:27 EST 2001
  !! SLOPES provides the gradient for the solvent excluded surface (ses), contact part.
  ...
  subroutine readmasktemplate(template)
    ...
    enddo
    close(11)
  end subroutine readmasktemplate
  !!----------------------------------------------------------------!!
  subroutine masker_initcollars()
    savecollars=.true.
  end subroutine masker_initcollars
  !!----------------------------------------------------------------!!
  subroutine masker_getms(xyz,nat,ms,buried,gradient,start,SAS,SES)
  !! ======================== GETMS ====================================
  ...
  logical,intent(in),optional :: SAS
  real,optional,intent(out) :: SES
  logical :: SASonly=.false.
  !
  ...
  !!
  if(present(SAS)) SASonly=SAS
  !!
  sendgrad = present(gradient)  !! if a gradient is passed, get shifts, send back
  i = 0
  periodic = (lbox > 0.)
  !!====== WARNING!!!  periodic must be turned off when using sendgrad. ====
  ...
  if (sendgrad) then
    ...
    ! else
    !   write(*,'("MASKER::masker_getms: bug, gradient not allocated.")')
    !   stop 
    ! endif
  endif
  ...
  do iat=istart,nat
    !! Diagnostic
    !! write(*,*) atomlib(atype(iat))
    if (mod(iat,100)==0) write(*,'(a1,$)') '.'
    if (atype(iat)<=0) cycle   !! atoms that are flagged with negative numbers or unknown type
                               !! are simply ignored.
    if (atype(iat)>nattype) cycle    !! unknown atom type?
    ...
    do jat=1,nat
      ...
      if (dd > 0.) then
        ...
        call masker_getimask2(r1+rw,dd,r2+rw,theta,avec,imask,itheta)
      else
        write(*,*) '############## dd=',dd,avec(1:3)
        write(*,'("MASKER ERROR: Two atoms at the exact same position.")')
        write(*,'("MASKER ERROR: iat,jat=",2i5," xyz1=",3f8.2," xyz2=",3f8.2)') iat,jat,xyz(1:3,iat),xyz(1:3,jat)
    ...
    if (sendburied) buried(iat) = 100*x  !! 'buried' is really "percent exposed"
    x = sphere*x
    !! Diagnostic
    !! write(*,*) 'iat=',iat,'  sasa=',x
    ...
    if (drawing) then
      ...
      ires = getresidue(iat)
      call masker_drawsurface(13,xyz(1:3,iat),r1,iat,cmask,50.,"V",scale,ires)  !! output the sasa surface
    endif
    ...
    do k=1,nm
      ...
      if (drawing) then
        ...
        call masker_drawsurface(13,xyz(1:3,iat),r1,kat,bmask,50.,"E",scale)  !! output the edges 1 at a time
      endif
      ...
      !! write(*,'("Edgemask contains ",i9)') jarc
      ...
      if (theta < pi/2) then  !! saddle exists
        ...
        !! Diagnostic
        ! write(*,*) 'iat=',iat,'  arcfrac=',arcfrac, '  slope=',slope(itheta), '  inter=',interc(itheta),'    sasa=',x
        sasa = sasa + x
        sasanrg = sasanrg + x*atomlib(atype(iat))%w1  !! kJ/mol
        !! saddle
        y = (r1+rw)*sin(theta)
        ...
        sdlarea = x
        !! write(*,'("Saddlearea ",2(i4,1x,a4),2f9.3)') iat,atomlib(atype(iat))%name,kat,atomlib(atype(kat))%name,sdlarea,arcfrac
        ssasa = ssasa + x
        ...
        if (sendgrad) then
          ...
          !write(*,'("Contact shift: ",2i4,f9.4)') iat,kat,xx
          ...
          ! write(*,'("Toroidal shift: ",2i4,f9.4)') iat,kat,xx
          !! write(*,'("Toroidal derivs: ",7f9.2)') xx,sdlarea,dij
          call addgrad(gradient(1,iat),xyz(1,iat),xyz(1,kat),xx)
          call addgrad(gradient(1,kat),xyz(1,kat),xyz(1,iat),xx)
        endif
      else    !! theta > pi/2
    ...
    do k=1,nn
      ...
      do m=1,nn
        ...
        !    !! call waterbowl(iat,kat,jat,x,wxyz,xyz,nat,imask) !! also uses/returns emask, uses cvec
          ! write(*,'("tetrahedron ",3(3f8.3,3x))') veci,vecj,veck
        call tetrahedron(veci,vecj,veck,wxyz,d,iat,jat,kat,1)
          ! write(*,'("after tetrahedron ",3(4f8.3,3x))') veci,vecj,veck,wxyz
          ! stop
        ...
        !    !   !! dmask = emask
        !    !   call masker_drawsurface(13,wxyz(1:3),rw,iat,dmask, 10.,"B",1) !! output the surface
        !    ! endif
        !  endif
        ...
        !    !     !! dmask = emask
        !    !     call masker_drawsurface(13,wxyz(1:3),rw,iat,dmask, 11.,"B",1) !! output the surface
        !    !   endif
        !    ! else
        !    !   x = 2*x
        !    ! endif
    ...
    enddo
  enddo  !! iat=
  if (present(SES)) SES = sasa
  ms = sasanrg
  !! Diagnostic
  !write(*,*)
  !write(*,*) "Done with contact and toroidal."
  ...
  do iwat = 1,nwat
    ...
    !! diagnostic
    ! write(*,'("WATER: ",4i5)') iwat,iat,jat,kat
    ! write(*,*) water(iwat)%xyz(1:3),xyz(1:3,iat),xyz(1:3,jat),xyz(1:3,kat),amask 
    ...
    !! count bits in complete triangle (before removing intersections)
     !write(*,*) "After trianglemask"
    jtri = countbits(amask)
     !write(*,*) "After countbits"
    !!
    ...
    ! write(*,'("jtri, jxtri, jcap= ",3i8," nn=",i4," fract=",f8.2)') jtri, jxtri, jcap, nn,real(jxtri)/real(jtri)
    ...
    if (sendgrad) then
      ...
      ddjk = ddjk
      ddik = ddik
      !! write(*,'("============ striangle:",3i5,3f8.3,$)') iat,jat,kat,ddij,ddjk,ddik
      !! 
    ...
    if (jxtri /= jtri) then  !! then there was an intersecting surface (5th body)
      if (sendgrad) then
        ...
        ! abur = abur/acap
        ! write(*,*) "Fract Cap height=",d," fraction of cap buried=",abur, " Cap surface area",acap
        ...
        !! 
        !!write(*,'("============ dCapdDij:",3i8,f10.2,3f10.4)') iat,jat,kat,xx,ddij,ddjk,ddik
    ...
    !! if (sendgrad) write(*,'(2i7,f8.5,3f8.3)') jtri,jxtri,abur,ddij,ddjk,ddik  !! finishes the line.
    if (jxtri < 1) cycle
    ...
    if (drawing) then  !! do this to force drawing surface of bowls
      ...
      ! call masker_drawsurface(13,water(iwat)%xyz,rw,iwat,dmask, 50.,"B",1) !! output the surface
      call masker_drawsurface(13,water(iwat)%xyz,rw,iwat,amask, 50.,"B",scale) !! output the surface
      if (rendering) &
  ...
  !! ====================================================================
  !! REPORT the surface area breakdown, if desired (meaning if draw.and.masker_saying)
  y = sasa + ssasa + bsasa
  if (present(SES)) SES = y
  x = sasanrg + ssasanrg + bsasanrg
  ms = x
  if (masker_saying) then
     write(*,*)
     write(*,'("Total contact surface   = ",f9.2)') sasa
     write(*,'("Total toroidal surface  = ",f9.2)') ssasa
     write(*,'("Total reentrant surface = ",f9.2)') bsasa
     write(*,'("Total SES surface area  = ",f9.2)') y
     write(*,'("Total contact energy    = ",f9.2," kJ")') sasanrg
     write(*,'("Total toroidal energy   = ",f9.2," kJ")') ssasanrg
     write(*,'("Total reentrant energy  = ",f9.2," kJ")') bsasanrg
  ...
  subroutine addgrad(grad,at1,at2,x)
    ...
    v = u*x
    !! 
    !! write(*,'("addgrad: ",4f9.3,6f8.2)') x,v(1:3),at1(1:3),at2(1:3)
    grad = grad + v
  ...
  ! if (d > 0.) then !! R-handed. Use iat,jat,kat and -cvec  !! NEW: It's always R-handed
    ...
    if (abs(d) < rw) then  !! there's a hole in the bowl, get mask (don't reset global imask)
      ...
      if (d > 0) then  !! waterbowl called using i,j,k. use cvec. better yet, use global 'imask'
        ...
        dvec = -1*cvec
        jmask = masker_getimask(dvec)        ! dont need dvec after this?
      endif
  ...
  !! just its area. In the calling function (getms), the difference between this mask
  ...
  jv = jv + eps*dvec
  !! 
  !! write(*,'("Expandedtriangle: i j k=",9f9.4)') iv,jv,kv
  ...
  dvec = dvec/y
  !! 
  !! write(*,'("Expandedtriangle:",3f9.4)') dvec
  jmask = masker_getimask(dvec)
  amask = iand(masklib(:,NTHETA,jmask),amask)
  ...
  !!------------------------------------------------------------
  integer function masker_getimask(uvec)
  implicit none
  ...
  elseif (ipsi > npsi) then
    stop 'masker.mod - Bug: psi calculation: masker_getimask'
  else
    phi = atan2(uvec(2),uvec(1))
  endif
  !! Diagnostic
  !  write(*,'(a,3f6.2,2f6.1,2i5,f6.3)') 'masker_getimask: uvec, psi, phi, ipsi, iphi, dphi',uvec, psi, phi, ipsi, iphi
  dphi = 2*pi/real(nphi(ipsi))
  ! if (dphi == 0.0) stop 'BUGBUGBUGBUGBUG'
  iphi = mod(int((phi/dphi)+0.5) + nphi(ipsi),nphi(ipsi))
  masker_getimask = psiposit(ipsi) + iphi
  end function masker_getimask
  !!------------------------------------------------------------
  subroutine masker_drawsurface(iunit,cen,r,iat,amask,bb,ch,scale,ires)
  ...
  end subroutine masker_drawsurface
  !!==========================================================
  ...
  logical :: cheating=.false.  !! use raster3D sphere object instead
  !! Diagnostic
  ! write(*,*) "RENDERSPHERE",cen
  !!--------------- BUG in triangles.f90 ----------
  !! this function is temporarilly shut down
  return
  !!
  ...
  do j=1,ntri
    if ((btest(amask((tri(1,j)-1)/NBIT + 1),mod(tri(1,j)-1,NBIT))).or.  &
      ...
      write(runit,*) 1
      write(runit,'(12f8.3)') avec,bvec,cvec,rgbtype
      if (smoothing) then
        write(runit,*) 7
  ...
  if (nomask) then  !! don't use edgemask
    ...
    !! write(*,'("dtauj dtauk: ",2f9.4)') dtauj,dtauk
    vv = wat + wij
    ...
    do itau=1,nsect
      ...
      do ivert=1,nvert
        ...
        !! write(*,'("Rotating ",3f9.4," to ",3f9.4,f9.2," degrees.")') norm(1:3,j),vv,dtauk/rad
        norm(1:3,i) = vv
        vert(1:3,i) = wat - r*norm(1:3,i)
        ...
        if (intrs) then
          ...
          do k=1,3
            ...
            !!write(*,'(a10,i3,7f8.3)') "dotprod", k,vv,vijk,dotprod(vv,vijk)
          enddo
          if (flg>0) cycle
    ...
    !! write(*,'("renderstrianlge: fract of triangle rendered=",f8.2)') (real(nout)/real(np))
    return   
  endif
  !! using mask
  do j=1,ntri
    ...
    elseif (nvert==2) then
      ...
      elseif (ip == 1) then  ! crosses one plane
        ...
        !  write(*,'("io:",2i5,5x,6f9.4)') jvert,ivert, vio(1:3),vp(1:3)
        call cros(vp,vio,vv)
        vv = vv/sqrt(dotprod(vv,vv))
        ! write(*,'("I:",2i5,3f9.4,5x,3f9.4)') k,ivert,mxyz(1:3,tri(ivert,j)), vv(1:3)
        cvec = cen + r*vv
        cnor = -vv
        call renderonetriangle(avec,bvec,cvec,anor,bnor,cnor,rgbv)
        ...
        call cros(vp,vio,vv)
        vv = vv/sqrt(dotprod(vv,vv))
        ! write(*,'("K:",2i5,3f9.4,5x,3f9.4)') k,kvert,mxyz(1:3,tri(kvert,j)), vv(1:3)
        avec = cen + r*vv
        anor = -vv
  ...
  do itau=1,ntau           ! steps separated by spcng
    ...
    do j=20,360,20
      ...
      !! write a triangle
      write(runit,*) 1
      write(runit,'(9f10.3,3f6.2)') avec,bvec,cvec,rgb(1:3)
      if (smoothing) then
        write(runit,*) 7
        write(runit,'(9f10.3)') trn(1:3,1),trn(1:3,2),trn(1:3,3)
        if (coloring /= 0 ) then
          write(runit,*) 17
          write(runit,'(9f10.3)') rgbv(1:3,1),rgbv(1:3,2),rgbv(1:3,3)
        endif
      endif
      ...
      if (smoothing) then
        write(runit,*) 7
        write(runit,'(9f10.3)') trn(1:3,2),trn(1:3,3),trn(1:3,4)
        if (coloring /= 0 ) then
          write(runit,*) 17
          write(runit,'(9f10.3)') rgbv(1:3,2),rgbv(1:3,3),rgbv(1:3,4)
        endif
      endif
...
!!    !! write a triangle
!    write(runit,*) 1
!    write(runit,'(9f10.3,3f6.2)') avec,bvec,cvec,0.0,1.0,0.0
!    if (smoothing) then
!      write(runit,*) 7
!      write(runit,'(9f10.3)') trn(1:3,1),trn(1:3,2),trn(1:3,3)
!    endif
!    !! write(*,'(a,6f10.3)') "last a,b ",trv(:,mod(nn+1,3)+1),trv(:,nn)
!!  enddo
...
!!==========================================================
  ...
  !!==========================================================
  subroutine masker_renderheader(cen,scale)
  !! This is the header data for a Raster3D file (*.r3d)
  !! ASSUMES runit IS OPEN
  real,dimension(3),intent(in) :: cen
  ...
  write(runit,'(a,$)') 'Raster3D file, generated by MASKER  '
  write(runit,'(i2,"-",a3,"-",i4))') values(3),month(values(2)),values(1)
  ...
  end subroutine masker_renderheader
  !!==========================================================
  ...
  x = diw*cos(ajiw)
  yp = diw*cos(akiw)
  ! write(0,*) "yp/sin(ajik) - x/tan(ajik)",yp,sin(ajik) , x,tan(ajik)
  if (ajik < TINYANG) return     !! this condition added to avoid divide by near-zero.  C.B. 16-JUN-2008
  y = yp/sin(ajik) - x/tan(ajik)
  ! write(0,*) "diw,x,y(",diw,x,y
  ! write(0,*) "sqrt(",(diw*diw - x*x - y*y)
  yp = diw*diw - x*x - y*y
  ...
  if (hand < 0) d = -d
  wxyz = (/x,y,d/)
  !!  write(0,*) "In tetrahedron: wxyz=",wxyz
  ...
  !!----------------------------------------------------------------!!
  ! VoidMask code fragment, using getwaters() output x and y
  !do i=1,MAXJ
  !  if (.not.any(sqrt( (x(1,i)-y(1,:))**2 + (x(2,i)-y(2,:))**2 + (x(3,i)-y(3,:))**2 ) < cutoff)) write(*,'(i5,3f8.3)') i,x(1:3,i)
  !enddo
  subroutine masker_getwaters(watout,nw)
    !! For use in VoidMask.f90
    !! CB Sun Jul  2 11:28:42 EDT 2006
    real,dimension(:,:),pointer :: watout
    ...
    enddo
    !! Diagnostic
    ! write(*,'("masker_getwaters(), nw=",i9)') nw
  end subroutine masker_getwaters
  !!----------------------------------------------------------------!!
  ...
  !! x = sig*ran(nseed)
  x = sig*x
  !write(*,*) "in ranvec",mxyz(1:3,imask)
  vec = x*mxyz(1:3,imask)
  end subroutine ranvec
  !----------------------------------------------------!
  subroutine masker_getimask2(side1,side2,sideopp,thet,uvec,imask,itheta)
  implicit none
  ...
  elseif  (thet > pi/2) then  !! partly embedded, get mask index in the opposite direction.
    ...
    imask = masker_getimask(-uvec)  !! uvec must be a unit vector
    ...
    if (itheta == NTHETA) then  
      imask = masker_getimask(-uvec)  !! close to being embedded. Use inverse mask for edges.
    else
      imask = masker_getimask(uvec)  !! uvec must be a unit vector
    endif
  endif
  end subroutine masker_getimask2
  !----------------------------------------------------!
  subroutine masker_setcoloring(str)
    !! integer,private,parameter :: surfacetension=1,byatom=2,byshape=3,bycharge=4
    implicit none
    character(len=*) :: str
  ...
  end subroutine masker_setcoloring
  !----------------------------------------------------!
  ...
  !----------------------------------------------------!
  subroutine masker_getmxyz(maskxyz)
    ! real,dimension(:,:),allocatable,private :: mxyz
    real,dimension(:,:),pointer :: maskxyz
    integer :: ios=0
    character(len=200) :: template
    !! if (.not.allocated(mxyz)) stop 'MASKER:: error. tried to get data from unallocated array mxyz.'
    if (.not.allocated(mxyz)) then
      call getenv("MASKTEMPLATE",template)
      if (template=="") call maskersetuperror("MASKTEMPLATE")
      call readmasktemplate(template)
    endif
    if (associated(maskxyz)) nullify(maskxyz)
    allocate(maskxyz(3,MAXATOM),stat=ios)
    if (ios/=0) stop 'masker.f90 :: masker_getmxyz -- unable to allocate maskxyz'
    maskxyz(1:3,1:MAXATOM) = mxyz(1:3,1:MAXATOM)
  end subroutine masker_getmxyz
  !----------------------------------------------------!
  subroutine masker_getvmask(amask)
    integer(kind=KND),dimension(MASKSIZE),intent(out) :: amask
    amask = vmask
  end subroutine masker_getvmask
  !----------------------------------------------------!
  subroutine masker_render(bool,unit,filename,smooth)
    !! generate raster3d files
    ...
    rendering = bool
    runit=14
    if (present(unit)) runit=unit
    if (present(smooth)) smoothing=smooth
    if (present(filename)) then
      open(runit,file=filename,status='replace',form='formatted',iostat=ios)
      if (ios/=0) stop 'masker.f90:: Error opening file for output. Permissions?'
    else
      open(runit,file='junk.r3d',status='replace',form='formatted',iostat=ios)
      if (ios/=0) stop 'masker.f90:: Error opening file for output. Permissions?'
    endif
  end subroutine masker_render
  !----------------------------------------------------!
  subroutine masker_plot(bool)
    logical,intent(in) :: bool
    drawing = bool
  end subroutine masker_plot
  !----------------------------------------------------!
  logical function masker_plotting()
    masker_plotting = drawing
  end function masker_plotting
  !----------------------------------------------------!
  subroutine masker_deallo()
    integer :: ios=0
    if (allocated(masklib)) deallocate(masklib, stat=ios)
    if (allocated(collarsize)) deallocate(collarsize, stat=ios)
    if (allocated(slope)) deallocate(slope, stat=ios)
    if (allocated(interc)) deallocate(interc, stat=ios)
    if (allocated(psiposit)) deallocate(psiposit, stat=ios)
    if (allocated(tri)) deallocate(tri, stat=ios)
    if (allocated(maskpsi)) deallocate(maskpsi, stat=ios) 
    if (allocated(maskphi)) deallocate(maskphi, stat=ios) 
    if (allocated(atomlib)) deallocate(atomlib, stat=ios)
    ...
    !! if (associated(maskxyz)) deallocate(maskxyz,stat=ios)
  end subroutine masker_deallo
  !----------------------------------------------------!
  pure real(kind=8) function iiand(amask,bmask)
    !! this routine packs 4 2-byte integers into one double
    !! for and-ing. Is it more efficient that 4 and's of 2-byte ints?
    !! CB Wed Jun  6 15:47:04 EDT 2007
    integer(kind=KND),dimension(4),intent(in) :: amask,bmask
    integer(kind=8) :: adble, bdble
    adble = transfer(amask,adble)
    bdble = transfer(bmask,bdble)
    iiand = iand(adble,bdble)
  end function iiand
  !----------------------------------------------------!
  ...
  !----------------------------------------------------!
  subroutine masker_setlbox(LL)
    real,intent(in) :: LL
    lbox = LL
  end subroutine masker_setlbox
  !----------------------------------------------------!
  integer function masker_nmask()
    masker_nmask = nmask
  end function masker_nmask
  !----------------------------------------------------!
  real function masker_dpsi()
    masker_dpsi = dpsi
  end function masker_dpsi
  !----------------------------------------------------!
  subroutine maskersetuperror(str)
    character(len=*),intent(in) :: str
    write(0,'("masker.f90:: getend ERROR ",a)') trim(str)
    write(0,'("Please run masker_setup.csh " )')
    stop 'Please run masker_setup.csh'
  endsubroutine maskersetuperror
  !----------------------------------------------------!
end module masker
