...
integer,dimension(:,:),allocatable :: cij, hx
real,dimension(:),allocatable :: dummy
character(len=200) :: cijfile,seqfile,psfile,str,eijfile,hxfile
...
jarg=iargc()
if (jarg < 5) then
  write(*,'("Usage: xpathway2ps seqfile agefile cijfile psfile hxfile [ilocal RT start B/W] ")')
...
call read_hx(hxfile,hx,nres)
call WRITEPS(psfile,eij,nres,nres,hx)
...
subroutine read_hx(hxfile,hx,nres)
  implicit none
  character(len=1) :: hx_color
  integer,dimension(:,:),allocatable,intent(out) :: hx
  character(len=80),intent(in) :: hxfile
  integer :: dunit,i,j
  integer,intent(in) :: nres

  if(allocated(hx)) deallocate(hx)
  allocate(hx(nres,nres))
  hx = 0
  write (*,*) hxfile
  open(newunit=dunit,file=hxfile,status='old',form='formatted',iostat=ios)
  if(ios /= 0) stop "Could not open hxfile"
  do
    read(dunit,'(i4,x,i4,x,a1)',iostat=ios)  i,j,hx_color
    if(ios /= 0) exit
    select case(hx_color)
    case ("R")
      hx(i,j) = 1
      hx(j,i) = 1
    case ("G")
...
end subroutine read_hx


subroutine WRITEPS(filename,eij,N,M,hx)
! Max values set to black, min set to white
implicit none
character(len=80),intent(in) :: filename
integer,intent(in) :: N,M
integer,dimension(:,:),allocatable,intent(in) :: hx
  ...
  do I=1,M
    ...
    if (I > J) then !! draw lower right triangle
      ...
      elseif (g < 0.997) then
        ...
        case(1)
          ! write(*,*) i,j,hx(i,j)
          write(22,'(3f6.3,f8.2," cm ",f8.2," cm  hsbbox")') 1.0,1.0,1.0,x,y
        case(2)
          ! write(*,*) i,j,hx(i,j)
          write(22,'(3f6.3,f8.2," cm ",f8.2," cm  hsbbox")') 0.35,1.0,1.0,x,y
        case(3)
          ! write(*,*) i,j,hx(i,j)
          write(22,'(3f6.3,f8.2," cm ",f8.2," cm  hsbbox")') 0.6,1.0,1.0,x,y
        case default
          ! write(*,*) i,j,hx(i,j)
          if ((I-J) == 3) then
