...
# awk '{printf "%9.1f %9.1f\n", $3, $4}' ${GNAME}.dag.nrg > ${HNAME}.plot
echo -n "" > ${HNAME}.plot
foreach ISEG (`grep ^"ISTATE " ${GNAME}.dag.nrg | awk '{print $5}'`)
  grep ^"ISEGNRG " ${GNAME}.dag.out | \
  awk '$2=='$ISEG' {printf "%9.2f %9.2f \n", $3, -log(($4+0.0000000000001)/(1.-$4))}' >> ${HNAME}.plot
end
...
foreach ISEG (`grep ^"ISTATE " ${GNAME}.dag.nrg | awk '{print $5}'`)
  grep ^"ISEGNRG " ${GNAME}.dag.out | awk '$2=='$ISEG | \
