module seams_main
	...
	integer, parameter :: MIN_CONTACTS_REGION=15  !! Minimal number of contacts for regions 
	integer, parameter :: MIN_CONTACTS_BETAS=5	  !! Minimal number of beta contacts 
...
!-seams_main-------------------------------------------------------------------
	...
	!			 (betas, helix, coils), and removes the ones that do not have contiguous 
	...
	subroutine getBetaSheetsPdb (pdbFilename, betaSheetSeq)
		...
		call writeMatrix (contactMatrix, "in.mat")
		...
		call writeMatrix (contactMatrix, "out.mat")
	endsubroutine
  ...
  !-----------------------------------------------------------------------
	...
	subroutine loadEnergyToMatrix (energyFilename, energyMatrix, nResidues)
		...
		do
			...
			read(aline, *,iostat=ios) aa1, aa2, energy, x
            if (ios/=0) then
               write(0,*) trim(aline)
               stop
            endif`
	...
	subroutine getBetaSheetGraph (betaSheetSeq, betaSheetContactsSeq, graphSeq)
		implicit none
		type(sequence_node), pointer, intent (in) :: betaSheetSeq		 ! Sequence of beta sheets each one with its set of contacts
	...
	subroutine detectBetaSheets (contactMatrix, betaResidues, minContacts, betaSheetSeq)
		...
		do i=1, n
			do j=1, n
				if (existsSeq (visitedSeq, (/j, i/)) .or. contactMatrix (j, i) == 0) then 
					cycle 
				endif
